...
dd <- d[complete.cases(d$rgdppc_2000),]#170个国家有观测值的2000的GDP数据，取对数由于国与国之间的财富差距大，取对数之后是的差距分布更平均
...
（1）参数问题，比如$\sigma$，与是否是非洲国家无关，如果把数据分开，将导致拟合
样本减少，从而损坏这些共同参数的估计。
...
**注意**：不能进通过后验均值和标准差表格来理解交互效应的主要原因有以下三点：

（1）在模型中添加交互效应时，同时改变了模型参数的意义

（2）由于表格数据中没有展示参数中的协方差，所以表格数据让我们很难完全理解其反应 的不确定性
